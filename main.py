...
class GameWidget(Widget):
    def __init__(self, **kwargs):
        ...
        self._life_label = CoreLabel(text="Lives: 50", font_size=20)
        self._life_label.refresh()
        self._life = 50
        ...
        with self.canvas:
            ...
            self._life_instruction = Rectangle(texture=self._life_label.texture, pos=(
                0, Window.height - 25), size=self._life_label.texture.size)
    ...
    def spawn_enemies(self, dt):
        for i in range(5):
            random_x = random.randint(0, Window.width - 50)
            y = Window.height
    ...
    def life(self, value):
        self._life = value
        self._life_label.text = "Life: " + str(value)
        self._life_label.refresh()
        self._life_instruction.texture = self._life_label.texture
        self._life_instruction.size = self._life_label.texture.size
...
class Laser(Entity):
    def __init__(self, pos, speed=300):
        super().__init__()
        sound = SoundLoader.load("assets/laser.mp3")
        sound.play()
        self._speed = speed * 2
        self.pos = pos
        self.source = "assets/laser.png"
...
class Explosion(Entity):
    def __init__(self, pos):
        super().__init__()
        self.pos = pos
        sound = SoundLoader.load("assets/explosion.mp3")
...
class Player(Entity):
    def __init__(self):
        ...
        game.bind(on_frame=self.move_step)
        self._shoot_event = Clock.schedule_interval(self.shoot_step, 0.25)
        self.pos = (400, 0)
    ...
    def move_step(self, sender, dt):
        ...
        newx = self.pos[0]
        newy = self.pos[1]
        if "left" in game.keysPressed:
            newx -= step_size
        if "right" in game.keysPressed:
            newx += step_size
        self.pos = (newx, newy)
...
game = GameWidget()
game.player = Player()
game.player.pos = (Window.width - Window.width/2, 0)
